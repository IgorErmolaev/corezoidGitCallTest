var black_cust_id=data.APP_CUST_ID_JUR;

function is_array (a) {
    return (typeof a == "object") && (a instanceof Array);
}

if ( black_cust_id==data.APP_CUST_ID_JUR ){
...
if (data.doc.r.o.BLN_BYCLIENTID != undefined) {
    ...
    if (BLN_BYCLIENTID.length>0) {
        ...
        const BL_ZONE_RED_CODE = [1,5,16,18,22,23,24,27,28,31,33,34,35,36,40,41,42,43,44,45];
        const BL_ZONE_RED_SOURCE = [5,22,34,35,39,53];
        const BL_ZONE_YELLOW_CODE = [2,3,4,6,9,10,11,14,21,26,29,30,32,38,39];
        const BL_ZONE_GREEN_CODE = [7,8,12,13,15,17,19,20,25,37,99];
        const BL_ZONE_GREEN_SOURCE_CODE = {57:40};

        const BL_CONTROL_ZPDO_CODE = [2,3,4,6,9,10,11,14,26,29,30,32,38,39];
        const BL_CONTROL_ZPDO_VALUE = "ZPDO;";
        const BL_CONTROL_DEAD_CODE = [7];
        const BL_CONTROL_DEAD_VALUE = "DEAD;";
        const BL_CONTROL_KHS_CODE = [2,4,6,9,10,11,14,21,26,29,30,32,38,39];
            ...
            if (curr_bl["@CtoBSt"] == "A") {
                ...
                if (curr_bl.TYPES != undefined) {
                    ...
                    for (var types_idx=0; types_idx < currTypes.length; types_idx++) {
                        ...
                        if (( (BL_ZONE_RED_CODE.indexOf(bl_code) > -1) || (bl_code > 45 && bl_code <=98) ||
