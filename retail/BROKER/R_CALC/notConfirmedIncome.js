...
if (data.PROD_CHAR_BANK == 'AB') {
  ...
  if (addrEstateType.indexOf(data.APP_ACT_ADDRESS_ESTATETYPE) != -1) {
    ...
    case data.RES_AGE>=43 && data.RES_AGE<=54.99:  data.LOCAL_DISP_SCORE_TABLE [2] = 0.06322; break;
    case data.RES_AGE>=38 && data.RES_AGE<=42.99:  data.LOCAL_DISP_SCORE_TABLE [2] = 0.07529; break;    
    case data.RES_AGE>=32 && data.RES_AGE<=37.99:  data.LOCAL_DISP_SCORE_TABLE [2] = 0.13074; break;
    case data.RES_AGE>=26 && data.RES_AGE<=31.99:  data.LOCAL_DISP_SCORE_TABLE [2] = 0.10048; break;
    case data.RES_AGE>=22 && data.RES_AGE<=25.99:  data.LOCAL_DISP_SCORE_TABLE [2] = 0.05482; break;
...
//-------неподтвержденнный доход для новых клиентов брокера с убки
if (data.PROD_CHAR_TYPE == 'BROKER' && data.LOCAL_CLIENT_NEW == 'Y') {

 var limitUbkiPb;
 data.LOCAL_LIMIT_UBKI_PB = 0; 
 
 if (data.UBKI_CRED != undefined && data.UBKI_CRED.length != 0) {
   for (var i = 0; i < data.UBKI_CRED.length; i++) {
    if (data.UBKI_CRED[i].PRODUCT == 'CC' && (data.UBKI_CRED[i].CREDIT_NUM.substring(0, 5) == "SAMPB" || data.UBKI_CRED[i].CREDIT_NUM.substring(0, 5) == "SAMDN") && data.UBKI_CRED[i].STATE == 'A') {
      limitUbkiPb = data.UBKI_CRED[i].LIMIT;
      data.LOCAL_LIMIT_UBKI_PB = Math.max(data.LOCAL_LIMIT_UBKI_PB,limitUbkiPb); 
    }
   }
 }
  data.LOCAL_LIMIT_NOT_CONF = data.LOCAL_LIMIT_UBKI_PB * 0.6;
  data.RES_INC_NOT_CONF = Math.max(data.RES_INC_NOT_CONF,data.LOCAL_LIMIT_NOT_CONF);
}
