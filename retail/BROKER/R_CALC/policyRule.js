...
if ((data.declNum < BAD_SCORE_DECIL && (data.LOCAL_DONBASS_EXCLUDES == 'Y' ||  data.LOCAL_CRED_HIST_VOSTOK == 'Y')) ||
    (data.declNum >= BAD_SCORE_DECIL && data.RES_SCCARD_SCORE_1<data.LOCAL_SCORE_BAL_OTS && data.LOCAL_DONBASS_EXCLUDES == 'Y')) { /*&& corpG.indexOf(data.PROD_CHAR_CORPORATION) != -1*/
    Donbass = 'True';
}
else {
    Donbass = 'False';
...
if (
    ...
    (data.LOCAL_SCOR_FLAG == 'Y' && data.RES_FINAL_KRED_SUM < data.PROD_SCHEME_MIN_AMOUNT_INT)
) {
...
if (data.RES_CRED_HIST_YBCH == 'NEGATIVE' && data.RES_DEBCARD_Z_SRED< 1000 && data.RES_DEBCARD_P_SRED<1000 && data.LOCAL_RESTRUCT != 'Y')
{
...
if (
    ...
    (data.LOCAL_DONBASS == 'Y' && Donbass == 'False')*/
)
{
