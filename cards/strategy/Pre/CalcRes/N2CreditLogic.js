...
for (var i=0; i<data.DATA_CRED.length;i++){
    if (data.DATA_CRED[i].REFERENC == data.PROD_CHAR_REFERENCE && data.PROD_CHAR_TYPE == 'UPLIMNKK'){
        ...
        data.RES_CRED_LIM = data.DATA_CRED[i].LIMIT;
        data.RES_LIMIT_START = data.DATA_CRED[i].START_SUMM;
    }
}

if (data.RES_LIMIT_START == 0 && data.PROD_CHAR_TYPE != 'UPLIMNKK' && data.DATA_CARD_UPGRADE_LIMIT > 0){
    data.RES_LIMIT_START = data.DATA_CARD_UPGRADE_LIMIT;
}
...
for (var i=0; i<data.DATA_CRED.length;i++){
    ...
    if (data.DATA_CRED[i].DATE_FIRSTPAY != null && data.DATA_CRED[i].DATECLOS_F != null) {
