var fs = require('fs');
var json = fs.readFileSync(__dirname+'\\resource\\ekb_nlink_resp_example.json').toString();
...
if (data.doc.r.o.NLINK_LST != undefined && data.doc.r.o.NLINK_LST.Link !=undefined) {
    ...
    for (var i=0; i<nlink.length; i++) {
        ...
        var link = new Object();
        if (data.APP_CUST_ID == Number(currlink["@MainClientId"])) {
            link.DIRECT = "M";
        ...
        /*
         link.LinkType = currlink["@LinkType"];
         link.NameKeyLeader = currlink["@NameKeyLeader"];
         link.NameKeyLinked = currlink["@NameKeyLinked"];
         */
        ...
        if ( linktype != undefined ) {
            ...
            if ( currlink["@LinkType"] == "0100400") {
                data.APP_MARITAL_ID = link.CUST_ID;
                data.LOCAL_ID_MARRIED = link.CUST_ID;
            }
        } else {
            link.TYPE="";
        }
