var fs = require('fs');
var json = fs.readFileSync(__dirname+'/../resource/bln_byclid_resp_example.json').toString();
...
/****************START*******************/
var black_cust_id=data.APP_CUST_ID;

function is_array (a) {
    return (typeof a == "object") && (a instanceof Array);
}

if ( black_cust_id==data.APP_CUST_ID ){
...
if (data.doc.r.o.BLN_BYCLIENTID != undefined) {
    ...
    if (BLN_BYCLIENTID.length>0) {
        ...
        const BL_ZONE_RED_SOURCE = [5,22,34,35,39,46,48,53]; //53 - для карт
        const BL_ZONE_RED_SOURCE_CODE = {59:15};
        const BL_ZONE_YELLOW_CODE = [2,4,6,9,10,11,14,21,26,30,32,38];
        const BL_ZONE_YELLOW_SOURCE_CODE = {53:15, 60:5};
        ...
        const BL_CONTROL_ZPDO_SOURCE = {60:5};
        ...
        const BL_CONTROL_KHS_SOURCE = {60:5};
        ...
        for (var i=0; i<BLN_BYCLIENTID.length; i++) {
            ...
            if (curr_bl["@CtoBSt"] == "A") {
                ...
                if (curr_bl.TYPES != undefined) {
                    ...
                    for (var types_idx=0; types_idx < currTypes.length; types_idx++) {
                        ...
                        if (( (BL_ZONE_RED_CODE.indexOf(bl_code) > -1)||(BL_ZONE_RED_SOURCE_CODE[bl_source] == bl_code) || (bl_code > 45 && bl_code <=98) ||
                            (BL_ZONE_RED_SOURCE.indexOf(bl_source) > -1)) && BL_ZONE_GREEN_SOURCE_CODE[bl_source] != bl_code && BL_ZONE_YELLOW_SOURCE_CODE[bl_source] != bl_code) {
                            bl.BL_ZONE=BL_ZONE_RED_VALUE;
                        } else if (BL_ZONE_YELLOW_CODE.indexOf(bl_code) > -1 || BL_ZONE_YELLOW_SOURCE_CODE[bl_source] == bl_code) {
                        ...
                        //проверка на control
                        //zpdo
                        if (BL_CONTROL_ZPDO_CODE.indexOf(bl_code) > -1 || BL_CONTROL_ZPDO_SOURCE[bl_source] == bl_code) {
                        ...
                        //khs
                        if (BL_CONTROL_KHS_CODE.indexOf(bl_code) > -1 || BL_CONTROL_KHS_SOURCE[bl_source] == bl_code) {
                ...
                //phone
                if (data.APP_PHONE != undefined && data.APP_PHONE.length > 0 && curr_bl["@BlackTable"] == "P") {
                    if ( black_cust_id==data.APP_CUST_ID ){
                        data.LOCAL_BLPHONE="Y";
                    }
                    var bl_num=curr_bl["@BlackId"];
                    for (var j=0; j<data.APP_PHONE.length; j++) {
                        if (data.APP_PHONE[j].NUMBER_CALL == bl_num) {
                            var bl_type=data.APP_PHONE[j].TYPE_CALL;
                            if (bl.BL_PHONE.indexOf(bl_type) == -1) {
                                bl.BL_PHONE = bl.BL_PHONE + bl_type + ";";
                            }
                        }
                    }
                }
        ...
        /*определение локалов для тони*/
        if (black_cust_id==data.APP_CUST_ID) {
            data.LOCAL_BLCL_TYPE_SMS = bl.SMS;
            data.LOCAL_BLCL_COLOR = bl.BL_ZONE;
            var stringToSplit = bl.BL_CONTROL;
            if ( stringToSplit[stringToSplit.length-1] == ';' ) stringToSplit = stringToSplit.substring(0,stringToSplit.length-1);
            data.LOCAL_BLCL_CONTROL = stringToSplit.split(";");
        }
